...
func main() {
	...
	var dsn = flag.String("dsn", "integers.db", "A valid BoltDB DSN string.")
	var bucket = flag.String("bucket", "integers", "A valid BoltDB bucket name.")

	flag.Parse()

	writer := io.MultiWriter(os.Stdout)

	logger := log.NewWOFLogger("[boltdb-integer-server] ")
	...
	pl, err := boltdb.NewBoltDBLIFOIntPool(*dsn, *bucket)
